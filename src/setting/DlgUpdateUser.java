...
public class DlgUpdateUser extends javax.swing.JDialog {
    ...
    private boolean penyakit=false,obat_penyakit=false,dokter=false,jadwal_praktek=false,petugas=false,pasien=false,registrasi=false,tindakan_ralan=false,kamar_inap=false,
            ...
            catatan_observasi_restrain_nonfarma=false,catatan_observasi_ventilator=false,catatan_anestesi_sedasi=false,rspm_perencanaan_pemulangan_igd=false;
    ...
    */
    ...
    private void tampil(String user) {  
        ...
        try{    
            ...
            ps=koneksi.prepareStatement(
                ...
                "user.catatan_observasi_ventilator,user.catatan_anestesi_sedasi,user.rspm_perencanaan_pemulangan_igd from user where user.id_user=AES_ENCRYPT(?,'nur')");
            try {
                ps.setString(1,user);
                rs=ps.executeQuery();
                if(rs.next()){
                    ...
                    catatan_observasi_restrain_nonfarma=rs.getBoolean("catatan_observasi_restrain_nonfarma");catatan_observasi_ventilator=rs.getBoolean("catatan_observasi_ventilator");catatan_anestesi_sedasi=rs.getBoolean("catatan_anestesi_sedasi");rspm_perencanaan_pemulangan_igd=rs.getBoolean("rspm_perencanaan_pemulangan_igd");
                    setTampil();
                }       
    ...
    private void setTampil(){
        ...
        if("[M]Perencanaan Pemulangan IGD".toLowerCase().contains(TCari.getText().toLowerCase())){
            tabMode.addRow(new Object[]{false,"[M]Perencanaan Pemulangan IGD",rspm_perencanaan_pemulangan_igd});
        }
    ...
    private void Simpan2() {
        for(i=0;i<tbUser.getRowCount();i++){ 
            ...
            if("[M]Perencanaan Pemulangan IGD".equals(tbUser.getValueAt(i,1).toString())){
                Sequel.mengedit("user","id_user=AES_ENCRYPT('"+TKd.getText()+"','nur')","rspm_perencanaan_pemulangan_igd='"+tbUser.getValueAt(i,2).toString()+"'");
            }
